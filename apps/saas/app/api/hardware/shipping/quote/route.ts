...
export async function POST(req: Request) {
  try {
    ...
    const products = (await readJson(PRODUCTS_FILE, [])) as any[];
    const inv = (await readJson(INVENTORY_FILE, [])) as any[];
    ...
    // TODO: UPS/FedEx/DHL + duties/tax engine
    const base = 9.9;
    const perItem = 1.5;
    ...
    return json({
      ...
      in_stock: inStock,
      quote: { currency: "USD", shipping, eta_days: country === "US" ? [2, 5] : [5, 12], carrier: "mock" },
      note: "TODO: carrier + tax + duties",
    });
  } catch (e: any) {
    return json({ error: "Server error", detail: String(e?.message ?? e), module: "hardware_shipping_quote" }, 500);
  }
}
